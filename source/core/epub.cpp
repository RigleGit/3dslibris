...
BuildTocProxyMapFromHtmlScan(const std::string &xml,
  ...
  while (pos < xml.size()) {
    ...
    std::string id = NormalizeFragmentId(ExtractHtmlAttrValue(tag, "id"));
    if (id.empty())
...
static bool LooksLikeSvgWrapper(const std::string &path_hint,
  ...
  std::string lower_head = ToLowerAscii(head);
  return lower_head.find("<svg") != std::string::npos;
}
...
static bool DecodeDataUriImage(const std::string &href, std::vector<u8> *out,
  ...
  bool ok = DecodeBase64Bytes(payload, out, EPUB_COVER_MAX_ENTRY_BYTES);
  if (app) {
    char msg[128];
    snprintf(msg, sizeof(msg), "EPUB: SVG data-uri decode %s bytes=%u",
...
static bool ResolveSvgImageWrapperPayload(
  ...
  while (pos < xml.size()) {
    ...
    if (resolved.empty())
      continue;
    if (resolved == svg_path)
      continue;
    if (ReadZipEntryBinary(uf, resolved, out, EPUB_COVER_MAX_ENTRY_BYTES)) {
      if (resolved_path)
        *resolved_path = resolved;
      if (app) {
        char msg[192];
        snprintf(msg, sizeof(msg),
                 "EPUB: SVG wrapper href resolved %s bytes=%u",
  ...
  if (app) {
    char msg[192];
    snprintf(msg, sizeof(msg),
             "EPUB: SVG wrapper unresolved tags=%d hrefs=%d path=%s",
...
static bool IsLikelyContentItem(const epub_item *item) {
  ...
  if (item->media_type == "application/xhtml+xml" ||
      ...
      item->media_type == "application/xml" || item->media_type == "text/xml")
    return true;
...
static bool
FindChapterPageFromParsedHeadings(const std::vector<ChapterEntry> &chapters,
                                  const std::string &toc_title, u16 min_page,
                                  u16 *page_out) {
...
int epub(Book *book, std::string name, bool metadataonly) {
  ...
  if (EPUB_ENABLE_REAL_TOC_RESOLVE) {
    ...
    bool toc_loaded = false;
    if (book && book->GetApp()) {
      DBG_LOG(book->GetApp(), "EPUB: TOC resolve begin");
    }

    if (!parsedata.navid.empty() &&
        FindManifestItemPath(parsedata, parsedata.navid, folder,
                             toc_doc_path)) {
      if (book && book->GetApp()) {
        char msg[192];
        snprintf(msg, sizeof(msg), "EPUB: try NAV %s", toc_doc_path.c_str());
      ...
      if (ReadZipEntryText(uf, toc_doc_path, toc_xml,
        ...
        DBG_LOG(book->GetApp(), "EPUB: NAV skipped (size/format)");
      }
    }

    if (!toc_loaded && !parsedata.tocid.empty() &&
        FindManifestItemPath(parsedata, parsedata.tocid, folder,
                             toc_doc_path)) {
      if (book && book->GetApp()) {
        char msg[192];
        snprintf(msg, sizeof(msg), "EPUB: try NCX %s", toc_doc_path.c_str());
      ...
      if (ReadZipEntryText(uf, toc_doc_path, toc_xml,
        ...
        DBG_LOG(book->GetApp(), "EPUB: NCX skipped (size/format)");
      }
    }
      ...
      for (auto item : parsedata.manifest) {
        if (item->media_type == "application/xhtml+xml" &&
          ...
          if (ReadZipEntryText(uf, toc_doc_path, toc_xml,
            ...
            DBG_LOG(book->GetApp(), "EPUB: fallback NAV skipped");
          }
          if (toc_loaded)
            break;
        }
      ...
      for (auto item : parsedata.manifest) {
        if (item->media_type == "application/x-dtbncx+xml") {
          ...
          if (ReadZipEntryText(uf, toc_doc_path, toc_xml,
            ...
            DBG_LOG(book->GetApp(), "EPUB: fallback NCX skipped");
          }
          if (toc_loaded)
            break;
        }
    ...
    if (book && book->GetApp()) {
      ...
      DBG_LOG(book->GetApp(), msg);
      DBG_LOG(book->GetApp(), "EPUB: TOC resolve end");
    }
  } else if (book && book->GetApp()) {
  ...
  epub_data_delete(&parsedata);
  if (book && book->GetApp()) {
    DBG_LOG(book->GetApp(), "EPUB: parse end");
  }
  return rc;
}
...
int epub_extract_cover(Book *book, const std::string &epubpath) {
  ...
  if (!LocateZipEntrySafe(uf, book->coverImagePath, app, "COVER")) {
    unzClose(uf);
    return 3;
  }
  ...
  if (LooksLikeSvgWrapper(decode_path, decodebuf)) {
    if (app) {
      char msg[192];
      snprintf(msg, sizeof(msg), "EPUB: cover SVG wrapper detected %s",
               decode_path.c_str());
      DBG_LOG(app, msg);
    }
    std::vector<u8> resolved;
    std::string resolved_path;
    if (ResolveSvgImageWrapperPayload(uf, decode_path, decodebuf, &resolved,
                                      &resolved_path, app)) {
      ...
      if (app) {
        char msg[192];
        snprintf(msg, sizeof(msg),
                 "EPUB: cover SVG wrapper resolved src=%s bytes=%u",
                 decode_path.c_str(), (unsigned)decodebuf.size());
        DBG_LOG(app, msg);
      }
    } else if (app) {
      DBG_LOG(app, "EPUB: cover SVG wrapper unresolved");
    }
  }
  unzClose(uf);
