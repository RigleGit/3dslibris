...
#ifdef __3DS__
static int
pdf_is_leap_year(int year)
{
	return (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0);
}

static time_t
pdf_timegm_3ds(const struct tm *tm)
{
	...
	for (year = 1970; year < tm->tm_year + 1900; ++year)
		days += pdf_is_leap_year(year) ? 366 : 365;

	days += days_before_month[tm->tm_mon];
	if (tm->tm_mon > 1 && pdf_is_leap_year(tm->tm_year + 1900))
		days += 1;
	days += tm->tm_mday - 1;

	return (time_t)(days * 86400LL + tm->tm_hour * 3600 +
		tm->tm_min * 60 + tm->tm_sec);
}

#define timegm pdf_timegm_3ds
#endif
